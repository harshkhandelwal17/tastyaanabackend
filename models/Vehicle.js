...
const vehicleSchema = new mongoose.Schema({
  ...
  type: {
    type: String,
    enum: ['EV', 'Diesel', 'Petrol',"DIESEL","PETROL", 'CNG', 'Hydrogen', "Electric Hybrid", "Petrol + CNG", "PETROL+CNG", "Diesel + CNG", "DIESEL+CNG"],
    required: true,
    default: 'Petrol' // Default for most 2-wheelers
  },
  vehicleNo: {
  ...
  rateHourly: {
    ...
    fuelChargesperkm: { // Fuel charge per km for fuel-included bookings
      type: Number,
      default: 6,
      min: 0
    },
...
vehicleSchema.methods.calculateRate = function (duration, rateType, includesFuel = false) {
  ...
  if (['hourly', 'hourly_plan'].includes(rateType)) {
    ...
    if (duration >= 24 && !includesFuel) {
      ...
      // Calculate free km based on blocks used + remaining hours
      const kmPerHour = (this.rateHourly && this.rateHourly.kmFreePerHour) || 10;
      rateConfig.kmLimit = (blocks24Hr * freeKm24Hr) + (blocks12Hr * freeKm12Hr) + (Math.floor(remainingTime) * kmPerHour);
    ...
    if (duration >= 24) {
      ...
      if (fullDays >= 1 && remainingHours > 0) {
        // Use block logic for the remainder using 12hr plan rate
        const extra12HrBlocks = Math.floor(remainingHours / 12);
        const extraRemainingHours = remainingHours % 12;

        const rate12Hr = (this.rate12hr && this.rate12hr.baseRate) || 500;
        const blockCharges = extra12HrBlocks * rate12Hr;
        const hourlyCharges = extraRemainingHours * rateConfig.extraChargePerHour;
  ...
  // Fuel Logic - REMOVED: Now fuel checkbox is for tracking only, not pricing
  // Both fuel-included and non-fuel use same base rates
  fuelCharges = 0;
...
vehicleSchema.post('find', function(docs) {
  if (docs && Array.isArray(docs)) {
    docs.forEach(doc => {
      if (doc.rateHourly && doc.rateHourly.fuelChargesperkm === undefined) {
        doc.rateHourly.fuelChargesperkm = 6;
      }
      if (doc.rate12hr && doc.rate12hr.fuelChargesperkm === undefined) {
        doc.rate12hr.fuelChargesperkm = 6;
      }
      if (doc.rate24hr && doc.rate24hr.fuelChargesperkm === undefined) {
        doc.rate24hr.fuelChargesperkm = 6;
      }
    });
  }
});

vehicleSchema.post('findOne', function(doc) {
  if (doc) {
    if (doc.rateHourly && doc.rateHourly.fuelChargesperkm === undefined) {
      doc.rateHourly.fuelChargesperkm = 6;
    }
