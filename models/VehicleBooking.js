...
const vehicleBookingSchema = new mongoose.Schema({
  ...
  // For premium vehicles that require confirmation before payment
  requiresApproval: {
    type: Boolean,
    default: false,
  ...
  includesFuel: {
    type: Boolean,
    default: false,
    description: 'Whether fuel is included in the booking (all km charged at fuelChargesperkm rate)'
  },
  ratePlanUsed: {
    baseRate: Number,
    ratePerHour: Number,
  ...
  billing: {
    ...
    kmLimit: {
      ...
      default: 0,
      min: 0,
      description: 'Free KM limit included with the booking plan'
    },
    extraKmCharge: {
      type: Number,
      default: 0,
  ...
  depositCollectionMethod: {
    type: String,
    enum: ['online', 'at-pickup', 'not-required'],
    default: 'online',
    description: 'How deposit should be collected'
  },
  depositStatus: {
    type: String,
    enum: ['pending', 'collected-online', 'collected-at-pickup', 'not-required', 'refunded'],
    default: 'pending',
    description: 'Status of deposit collection'
  },
  ...
  refundDetails: {
    ...
    refundMode: {
      type: String,
      enum: ['cash', 'online'],
      description: 'Actual mode of refund given to customer'
    },
    bankDetails: {
  ...
  vehicleReplacements: [{
    vehicleId: {
      type: mongoose.Schema.Types.ObjectId,
      ref: 'Vehicle',
      description: 'Original vehicle that was replaced'
    },
    brand: String,
    model: String,
    ...
    replacedBy: {
      type: mongoose.Schema.Types.ObjectId,
      ref: 'User',
      description: 'Seller/Worker who performed the replacement'
    },
    reason: {
      type: String,
      default: 'Vehicle breakdown/maintenance issue'
    }
  }],
