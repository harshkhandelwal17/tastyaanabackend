...
const InstallPWA = () => {
  ...
  useEffect(() => {
    ...
    const handleBeforeInstallPrompt = (e) => {
      e.preventDefault();
      setDeferredPrompt(e);
      setShowPWA(true); // show PWA option
    };

    window.addEventListener("beforeinstallprompt", handleBeforeInstallPrompt);

    // Hide bar once app is installed
    ...
    <div className="bg-gradient-to-r from-emerald-500 via-emerald-600 to-emerald-700 text-white py-2.5 px-4 relative overflow-hidden shadow-lg">
      ...
      <div className="relative z-10 flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="flex items-center space-x-2 flex-shrink-0">
          ...
          {/* PWA Install Button (only show if PWA is available) */}
          {showPWA && (
            <button
              onClick={handlePWAInstall}
              className="bg-white/20 text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-white/30 transition-colors duration-200 flex items-center space-x-1 shadow-sm border border-white/30"
            >
