...
const MobileOrderCard = ({
  ...
  // Get current user ID to check if order has been viewed by this seller
  const { user: authUser } = useSelector((state) => state.auth);
  console.log("dksjbffyigbdsbdfh", order.status);
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      ...
      // Use different API endpoint for subscription orders
      if (order.isSubscriptionOrder || order.type === "gkk") {
        // Use tiffin status update API
        ...
        // Show special message for delivered subscription orders
        ...
        // Use regular order status update API
      ...
      // Mark order as viewed permanently in backend when status is updated
      try {
  ...
  const markAsViewed = () => {
    if (!isViewed) {
      setIsViewed(true);
      // You can add API call here to mark order as viewed if needed
    }
  };
  ...
  const isNewOrder = () => {
    ...
    return diffInMinutes < 60 && !hasBeenViewedByCurrentSeller; // New if less than 1 hour and not viewed by current seller
  };
      ...
      <div className="px-4 py-3">
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="flex items-center text-sm text-slate-700">
            ...
            <span className="font-medium">{getItemsText(order.items)}</span>
          </div>
          <div className="flex items-center text-sm text-slate-700">
          ...
          {statusOptions.length > 0 && (
            <div className="relative flex-1">
              <button
                ...
                disabled={isUpdating}
                className="w-full flex items-center justify-center px-3 py-2 text-sm font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isUpdating ? (
                  <>
...
const SellerOrders = () => {
  ...
  const { orders = [], totalCount = 0 } = ordersData?.data || {};
        ...
        <div className="bg-white border-b border-slate-200 px-4 py-3">
          ...
          <div className="relative mb-3">
            ...
            <input
              ...
              className="pl-10 w-full px-3 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
            />
          </div>
