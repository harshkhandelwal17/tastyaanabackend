...
if (isThirtyDayPlan) {
  ...
  baseMealAmountPerMeal = subscription.pricing.totalAmount / subscription.pricing.totalMeals;
        ...
        // REMOVED: Meal count deduction logic - now handled ONLY in updateDynamicDeliveryStatus function
        // This function only handles payment processing, not delivery tracking
        ...
        // REMOVED: Expiry check - handled centrally based on actual deliveries
...
const getUserSubscriptions = async (req, res) => {
  ...
  //  console.log("inside subscription controller ",req.user)
    ...
    .populate('sellerId', 'name sellerProfile.mealAvailability sellerProfile.businessName')
    .sort({ createdAt: -1 })
    .limit(limit * 1)
    .skip((page - 1) * limit)
    .lean();
    ...
    const updatedSubscriptions = subscriptions.map(sub => {
      ...
      // Get seller meal availability
      const sellerMealAvailability = sub.sellerId?.sellerProfile?.mealAvailability || {
        isAvailable: true,
        status: 'available',
        reason: null,
        shifts: {
          morning: { isAvailable: true, status: 'available', reason: null },
          evening: { isAvailable: true, status: 'available', reason: null }
        }
      };
      ...
      return {
        ...
        sellerMealAvailability: sellerMealAvailability
      };
    });
...
/**
 * @desc    Check if seller has marked "no meal today" for user's subscriptions
 * @route   GET /api/subscriptions/check-no-meal-today
 * @access  Private (User)
 */
const checkNoMealToday = async (req, res) => {
  try {
    const userId = req.user._id;
    ...
    console.log(`Checking no meal status for user: ${userId}, date: ${currentDate}`);
    
    // Parse the selected date for Indian timezone
    const [year, month, day] = currentDate.split('-').map(Number);
    const selectedDate = new Date(Date.UTC(year, month - 1, day - 1, 18, 30, 0, 0));
    ...
    // Check no meal status for each subscription's seller
    const noMealAlerts = [];
    
    for (const subscription of userSubscriptions) {
      const sellerId = subscription.sellerId._id;
      ...
      if (subscription.shift === 'both') {
        ...
        subscriptionShifts = ['evening']; // default
      }
      
      for (const shift of subscriptionShifts) {
        // Check if seller has marked no meal for this date and shift
        const noMealEntry = await DailyMeal.findOne({
          seller: sellerId,
          date: selectedDate,
          shift: shift,
        ...
        if (noMealEntry) {
          noMealAlerts.push({
            ...
            seller: subscription.sellerId,
            shift: shift,
            reason: noMealEntry.noMealReason || 'No meal available today',
            date: currentDate
          });
        }
      }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        noMealAlerts,
        totalAlerts: noMealAlerts.length,
        date: currentDate,
        message: noMealAlerts.length > 0 
          ? `Found ${noMealAlerts.length} no-meal alerts for today`
          : 'All your meal providers are serving today!'
      }
    });
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to check no meal status',
      error: error.message
    });
  }
};
