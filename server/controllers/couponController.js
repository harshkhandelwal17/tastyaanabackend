  ...
  validateCoupon: async (req, res) => {
    try {
      ...
      if (!coupon.isValid) {
        ...
        } else if (coupon.totalUsageLimit && coupon.usedCount >= coupon.totalUsageLimit) {
          reason = 'Total coupon usage limit exceeded';
        }
...
console.log("coupan is : ",coupon);
  ...
  /**
   * Get enhanced coupon usage statistics
   */
  getEnhancedUsageStatistics: async (req, res) => {
    try {
      const { id: couponId } = req.params;
      const userId = req.user._id;
      ...
      const statistics = await coupon.getUsageStatistics(userId);
      ...
      console.error('Error fetching enhanced usage statistics:', error);
      res.status(500).json({
        success: false,
