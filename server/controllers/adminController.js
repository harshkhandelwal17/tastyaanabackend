...
/**
 * @desc    Get all centers/zones with seller information for admin
 * @route   GET /api/admin/centers
 * @access  Private/Admin
 */
exports.getAdminCenters = async (req, res) => {
  try {
    ...
    if (status !== 'all') {
      zonesQuery.isActive = status === 'active';
    }

    const zones = await DeliveryZone.find(zonesQuery)
      .skip(skip)
      .limit(parseInt(limit))
      .sort({ createdAt: -1 })
      .lean();

    const totalZones = await DeliveryZone.countDocuments(zonesQuery);

    // For each zone, get sellers and vehicles
    const centersWithDetails = await Promise.all(zones.map(async (zone) => {
      // Find sellers with vehicle rental services in this zone
      const sellersInZone = await User.find({
        role: 'seller',
        'sellerProfile.vehicleRentalService.isEnabled': true,
        'sellerProfile.vehicleRentalService.serviceZones': {
          $elemMatch: {
            $or: [
              { zoneCode: zone.code },
              { zoneName: zone.name }
        ...
        email: 1,
        phone: 1,
        'sellerProfile.companyName': 1,
        'sellerProfile.vehicleRentalService.serviceStatus': 1,
        'sellerProfile.vehicleRentalService.businessType': 1,
        'sellerProfile.vehicleRentalService.fleetStats': 1,
        'sellerProfile.vehicleRentalService.serviceZones': 1
      }).lean();

      // Get vehicle count for this zone
      const Vehicle = require('../models/Vehicle');
      const vehicleStats = await Vehicle.aggregate([
        { $match: { zoneCode: zone.code } },
        {
          $group: {
            _id: null,
            totalVehicles: { $sum: 1 },
      ...
      return {
        ...zone,
        sellers: sellersInZone.map(seller => {
          const serviceZone = seller.sellerProfile?.vehicleRentalService?.serviceZones?.find(
            sz => sz.zoneCode === zone.code || sz.zoneName === zone.name
          );
          ...
          return {
            ...
            email: seller.email,
            phone: seller.phone,
            companyName: seller.sellerProfile?.companyName || 'N/A',
            serviceStatus: seller.sellerProfile?.vehicleRentalService?.serviceStatus || 'inactive',
            businessType: seller.sellerProfile?.vehicleRentalService?.businessType || 'individual',
            totalVehicles: seller.sellerProfile?.vehicleRentalService?.fleetStats?.totalVehicles || 0,
            activeVehicles: seller.sellerProfile?.vehicleRentalService?.fleetStats?.activeVehicles || 0,
            centerInfo: serviceZone ? {
              address: serviceZone.address,
              coordinates: serviceZone.coordinates,
              contactInfo: serviceZone.contactInfo,
              operatingHours: serviceZone.operatingHours
            } : {}
          };
        }),
        vehicleStats: stats,
        activeSellerCount: sellersInZone.filter(s => 
          s.sellerProfile?.vehicleRentalService?.serviceStatus === 'active'
        ).length
      };
    }));
    ...
    res.json({
      ...
      data: {
        ...
        pagination: {
          ...
          totalCenters: totalZones,
          limit: parseInt(limit)
        },
        summary: {
          totalCenters: totalZones,
          activeCenters: centersWithDetails.filter(c => c.isActive).length,
          totalSellers: centersWithDetails.reduce((sum, c) => sum + c.sellers.length, 0),
          totalVehicles: centersWithDetails.reduce((sum, c) => sum + c.vehicleStats.totalVehicles, 0)
        }
      }
    });

  } catch (error) {
    console.error('Error getting admin centers:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to get centers data',
      error: error.message
    });
  }
};
