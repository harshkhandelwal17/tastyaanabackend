...
const validateBookingDetails = async (req, res) => {
  try {
    ...
    if (durationHours > 168) { // 7 days
      return res.status(400).json({
        success: false,
        message: `Booking duration is ${Math.round(durationHours)} hours (${(durationHours / 24).toFixed(1)} days). Maximum booking duration is 7 days.`,
        error: 'DURATION_TOO_LONG',
        details: {
          requestedDuration: durationHours,
          maximumDuration: 168,
          suggestion: 'Please reduce your booking to 7 days (168 hours) or less.'
        }
      });
    }
...
// console.log('Received booking data:', bookingData);
    ...
    if (durationHours > 168) { // 7 days = 168 hours
      return res.status(400).json({
        success: false,
        message: 'Maximum booking duration is 7 days (168 hours).'
      });
    }
...
const verifyPayment = async (req, res) => {
  try {
    ...
    } else if (booking.bookingStatus === 'awaiting_approval') {
      // Keep availability as 'reserved' or similar until approved? 
      // If we block the slot (which we do in createBooking via 'reserved'), we are good.
      // createBooking sets it to 'reserved'. existing logic is fine.
    }
  ...
  try {
    ...
    if (action === 'approve') {
      ...
      booking.statusHistory.push({
        ...
        notes: 'Request approved by seller'
      });
