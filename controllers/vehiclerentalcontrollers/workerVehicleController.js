...
// ===== Collect Additional Payment During Booking (Worker) =====
const collectWorkerPaymentOnBooking = async (req, res) => {
  try {
    const workerId = req.user.id;
    const { bookingId } = req.params;
    ...
    if (isMixed) {
      ...
      if (cash > 0) {
        booking.payments.push({
          ...
          receivedBy: workerId,
          receivedAt,
          notes: notes ? `${notes} (cash portion)` : 'Cash portion of mixed payment collected by worker'
        });
      ...
      if (online > 0) {
        booking.payments.push({
          ...
          receivedBy: workerId,
          receivedAt,
          notes: notes ? `${notes} (online portion)` : 'Online portion of mixed payment collected by worker'
        });
      ...
      booking.payments.push({
        ...
        receivedBy: workerId,
        receivedAt,
        notes: notes || 'Additional payment collected by worker'
      });
    ...
    booking.statusHistory.push({
      ...
      updatedBy: workerId,
      updatedAt: new Date(),
      notes: `Payment of ₹${collectAmount} (${isMixed ? `Mixed: ₹${cashAmount || 0} cash + ₹${onlineAmount || 0} online` : normalizedType}) collected by worker`
    });
    ...
    console.error('Error collecting payment (worker):', error);
